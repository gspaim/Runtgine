...
// ExecFunc invokes `npm` with argv (never a shell string). args start at
// optional `--silent` then `test`.
...
func ValidateStaticInput(workspace, capability string, raw json.RawMessage) error {
	if capability != CapTest {
		return result.Validation(result.CodeUnknownCapability, "npm player cannot validate "+capability, nil)
	}
	var in input
	...
	if _, err := os.Stat(filepath.Join(resolved, "package.json")); err != nil {
		return result.Validation(result.CodeInvalidInput, "workdir must contain package.json", map[string]any{
			"workdir": resolved,
		})
	}
	return nil
...
func defaultExec(ctx context.Context, dir string, env, args []string) (string, string, int, error) {
	if _, err := exec.LookPath("npm"); err != nil {
		return "", "", -1, result.Runtime(result.CodePlayerError, "npm binary not found in PATH", false, nil)
	}
	...
	if ctx.Err() == context.DeadlineExceeded {
		return stdout.String(), stderr.String(), -1, result.Runtime(result.CodeTimeout, "npm.test timed out", true, map[string]any{
			"stderr": stderr.String(),
		})
	}
	if ctx.Err() == context.Canceled {
		return stdout.String(), stderr.String(), -1, result.Runtime(result.CodeCancelled, "npm.test cancelled", false, nil)
	}
	if runErr != nil {
