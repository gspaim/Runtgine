...
const (
	CapIdentity = "gcp.identity"
	CapConfig   = "gcp.config"
	CapProjects = "gcp.projects"
  ...
  "properties":{
	...
	return registry.Manifest{
		...
		Capabilities: []registry.Capability{
			...
			{Name: CapConfig, InputSchema: inSchema, OutputSchema: outSchema},
			{Name: CapProjects, InputSchema: inSchema, OutputSchema: outSchema},
		},
	}
}
...
func ValidateStaticInput(workspace, capability string, raw json.RawMessage) error {
	switch capability {
	case CapIdentity, CapConfig, CapProjects:
		var in input
	...
	default:
		return result.Validation(result.CodeUnknownCapability, "gcp player cannot validate "+capability, nil)
	}
}
...
func (p *Player) Execute(ctx context.Context, req registry.ExecRequest) (json.RawMessage, error) {
	...
	case CapIdentity:
		head = []string{"auth", "list"}
	case CapConfig:
		head = []string{"config", "list"}
	case CapProjects:
		head = []string{"projects", "list"}
	default:
		return nil, result.Validation(result.CodeUnknownCapability, "unknown capability "+req.Capability, nil)
	}
	args := append(head, "--format=json")
...
func (p *Player) runJSON(ctx context.Context, d time.Duration, args []string) (any, bool, error) {
	...
	if exit != 0 {
		...
		if msg == "" {
			msg = "gcloud exited with code " + strconv.Itoa(exit)
		}
		return nil, false, result.Runtime(result.CodePlayerError, "gcloud: "+msg, false, map[string]any{"exit_code": exit})
	}
	truncated := false
...
func defaultRunCmd(ctx context.Context, timeout time.Duration, args []string) (string, string, int, error) {
	if _, err := exec.LookPath("gcloud"); err != nil {
		return "", "", 0, result.Runtime(result.CodePlayerError, "gcloud binary not found in PATH", false, nil)
	}
	...
	if err != nil {
		if ee, ok := err.(*exec.ExitError); ok {
			...
			return stdout.String(), stderr.String(), 0, result.Runtime(result.CodeTimeout, "gcp: timeout", true, nil)
		}
	}
