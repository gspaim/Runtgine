...
INSERT INTO runs(run_id, task_id, status, error_json, task_json, created_at, updated_at)
VALUES(?,?,?,?,?,?,?)`,
		run.RunID, run.TaskID, string(run.Status), "", string(taskJSON), now, now)
	return err
}
...
SELECT run_id, task_id, status, error_json, task_json, created_at, updated_at
FROM runs WHERE run_id=?`, runID).Scan(
		&r.RunID, &r.TaskID, &status, &r.ErrorJSON, &taskJSON, &created, &updated)
	if err != nil {
		return r, nil, err
	}
